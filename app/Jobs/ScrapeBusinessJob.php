...
class ScrapeBusinessJob implements ShouldQueue
{
    ...
    public function handle(OutscraperService $service): void
    {
        ...
        try {
            $info = $service->getBusinessInfo($this->business->google_place_id);
            $reviews = $service->getReviews($this->business->google_place_id, 20);
        ...
        foreach ($reviews as $review) {
            $wasRecentlyCreated = $this->business->reviews()->updateOrCreate(
        ...
        $this->business->scrapeLogs()->create([
            'status' => 'success',
            'source' => 'outscraper',
        ...
        if ($this->business->status === 'pending_setup') {
    ...
    {
        if (! $this->business->scrapeLogs()->where('created_at', '>=', now()->subMinute())->exists()) {
